...
services:
  homeassistant:
    ...
    volumes:
      - /home/usuario/homeassistant/config:/config
  ...
  esphome:
    ...
    volumes:
      - /home/usuario/esphome/config:/config
      - /etc/localtime:/etc/localtime:ro
    environment:
      - TZ=Europe/Madrid
    restart: unless-stopped
...
El archivo de configuración \texttt{automations.yaml} aloja el motor de reglas lógicas que rigen el comportamiento inteligente de la estancia. Se detallan a continuación las diez automatizaciones operativas:
...
Regla de análisis local ejecutada diariamente a las 22:00:00. Evalúa los históricos de tiempo activo en el escritorio y las horas de descanso nocturno y genera un reporte analítico adaptativo mediante plantillas Jinja2. Para evitar ruidos visuales derivados de la precisión de punto flotante en las bases de datos lógicas, las lecturas temporales se formatean dinámicamente limitando su salida a dos cifras decimales redondeadas mediante filtros de conversión de datos. El informe resultante es transmitido por vía de notificación push al usuario sin contactar con servidores externos.
...
- id: 'informe_diario_qol'
  ...
  actions:
    - data:
        ...
        message: |
          Resumen de tu jornada: - Tiempo efectivo de estudio: {{ states(''sensor.tiempo_de_estudio'') | float(0) | round(2) }} horas. - Tiempo de descanso (anoche): {{ states(''sensor.tiempo_de_sueno'') | float(0) | round(2) }} horas.
          {% if states(''sensor.tiempo_de_estudio'') | float(0) > 4.0 %}
...
Esta regla infiere el paso de la estancia a modo de \texttt{Estudio} basándose en la distancia reportada por la telemetría del radar LD2410 del Nodo de Escritorio. Si el radar reporta la presencia estática de un cuerpo en un radio inferior a 1,5 metros durante más de 2 minutos, se conmuta el estado lógico general de la estancia a \texttt{Estudio}. Por contra, si se registra una distancia superior a 0,5 metros durante el mismo lapso, el sistema devuelve el contexto a \texttt{Ocio}.
...
\subsubsection{Retorno a la habitación desde ausencia}
Esta regla permite la salida reactiva del modo \texttt{Ausente}. Cuando el sensor de ocupación del radar del Nodo Escritorio conmuta a encendido (\texttt{on}) y el selector de estado de la estancia se encuentra bloqueado en \texttt{Ausente}, el motor lógico asume la reincorporación física del usuario a la habitación, desbloqueando el sistema al transicionar el contexto al estado general de \texttt{Ocio}. Esto permite que el resto de automatizaciones de inferencia adaptativa (como el sensor Bayesiano y la detección de estudio) vuelvan a operar normalmente.

\begin{lstlisting}[language=YAML, caption={Automatización para restablecer el estado tras la reincorporación al entorno.}, label={lst:auto_retorno}]
- id: '1779447855315'
  alias: 'Contexto: Vuelta a la Habitacion'
  description: 'Pasa la habitacion a Ocio al detectar presencia si estaba en Ausente.'
  trigger:
    - entity_id: 'binary_sensor.esp32_zona_escritorio_ocupacion_general'
      to: 'on'
      trigger: 'state'
