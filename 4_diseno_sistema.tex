...
\begin{table}[H]
    ...
    \begin{tabular}{|l|l|l|}
        ...
        \textbf{Nombre} & \textbf{Dato(s) de entrada} & \textbf{Dato(s) de salida / Acción} \\ \hline
...
\begin{enumerate}
    \item \textbf{Tabla \texttt{Nodo} (Primera Forma Normal - 1NF):} todos los atributos son atómicos (no repetitivos) y se identifican mediante una clave primaria única (\texttt{id\_nodo}).
    \item \textbf{Tabla \texttt{Sensor} (Segunda Forma Normal - 2NF):} cumple 1NF y no existen dependencias funcionales parciales; los atributos describen de forma exclusiva a la entidad sensor ligada por clave foránea a un nodo (\texttt{id\_nodo}).
    \item \textbf{Tabla \texttt{Lectura} y \texttt{EstadoContexto} (Tercera Forma Normal - 3NF):} cumple 2NF y se eliminan dependencias transitivas. El valor de la lectura de un sensor o el cambio de estado depende únicamente de la clave primaria, y no existe ningún atributo no primario dependiente de otro atributo no primario.
\end{enumerate}
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|l|l|l|}
        \hline
        \rowcolor[gray]{0.9} 
        \textbf{Tabla / Atributo} & \textbf{Tipo de dato} & \textbf{Restricción} & \textbf{Descripción} \\ \hline
