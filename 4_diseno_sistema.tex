...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        % Nodes with increased horizontal spacing (5.8cm) and vertical spacing (4.3cm)
        \node[block, fill=Naranja!10, draw=Naranja!80] (sensores) at (-5.8, 2.5) {Nodos Sensores\\(ESP32)};
        \node[block, fill=Verde!10, draw=Verde!80] (broker) at (0, 2.5) {Broker MQTT\\(Mosquitto)};
        \node[block, fill=Azul!10, draw=Azul!80] (ha) at (5.8, 2.5) {Orquestador\\(Home Assistant)};
        \node[block, fill=Morado!10, draw=Morado!80] (db) at (5.8, -1.8) {Persistencia\\(Base de Datos)};
        \node[block, fill=Rojo!10, draw=Rojo!80] (rele) at (-5.8, -1.8) {Actuador\\(Módulo Relé)};

        % Paths (perfectly separated, preventing overlaps)
        \draw[arrow] (sensores) -- node[above, txt] {Publica\\telemetría\\(QoS 0)} (broker);
        \draw[arrow] (broker) to[bend left=20] node[above, txt] {Distribuye\\eventos\\(Suscripción)} (ha);
        \draw[arrow] (ha) to[bend left=20] node[below, txt] {Publica\\comando\\(QoS 1)} (broker);
        \draw[arrow] (ha) -- node[right, txt] {Registra\\series\\temporales} (db);
        \draw[arrow] (broker) -- node[below left, txt] {Conmutación\\eléctrica} (rele);
    \end{tikzpicture}
    \caption{Diagrama de flujos de información y mensajería dirigida por eventos en la red local.}
