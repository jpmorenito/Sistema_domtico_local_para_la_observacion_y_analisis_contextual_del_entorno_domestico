...
\begin{itemize}
    ...
    \item \textbf{Vulnerabilidades lógicas delegadas en el perímetro de red:} Aunque el procesamiento local elimina los vectores de ataque externos orientados a la interceptación de datos en tránsito hacia la nube, la seguridad global del ecosistema se desplaza y delega por completo en la robustez del protocolo de cifrado de la red inalámbrica doméstica (WLAN). Debido a las restricciones de memoria y procesamiento de los microcontroladores empotrados ESP32 para gestionar de manera fluida capas de cifrado pesado (como TLS/SSL de extremo a extremo sobre el protocolo MQTT local), un compromiso de la seguridad de la red local expondría la telemetría del sistema a técnicas de monitorización interna.
\end{itemize}
