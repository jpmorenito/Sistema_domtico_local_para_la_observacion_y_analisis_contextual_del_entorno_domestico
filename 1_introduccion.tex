...
\begin{itemize}
    \item El diseño, ensamblaje y puesta en marcha de tres nodos de hardware distribuidos basados en la plataforma ESP32, equipados con sensórica biométrica y ambiental (FMCW, LDR, transductores climáticos, acústicos, etc.).
...
\begin{itemize}
    ...
    \item \textbf{Desarrollo del motor de contexto:} programar un servidor central centralizado que, mediante técnicas de fusión de datos, asigne un estado unívoco a la habitación (ocio, estudio, durmiendo, ausente) en tiempo real.
    \item \textbf{Automatización proactiva y seguridad:} diseñar algoritmos de control lógico para la gestión ergonómica de la iluminación, la evaluación del descanso y el despliegue de un sistema de alarma sólido frente a vulnerabilidades lógicas, como las condiciones de carrera.
...
\begin{enumerate}
    ...
    \item \textbf{Gestión centralizada y fusión de datos en el Edge:} implementación de las máquinas de estado y reglas lógicas redundantes en el nodo central para dotar al sistema de tolerancia a fallos ante pérdidas temporales de conectividad o lecturas anómalas de los transductores.
...
\end{enumerate}
    ...
    \node[box] (integ) at (7.5, 2.5) {Integración y\\gestión centralizada};
...
Con el fin de guiar al lector, la presente memoria se ha estructurado de manera progresiva. Comienza con el Capítulo 1, donde se introduce la motivación del proyecto, la problemática de la privacidad en la domótica actual y los objetivos perseguidos. A continuación, el Capítulo 2 aborda la evolución del estado del arte, prestando especial atención a la transición hacia el procesamiento local y a los fundamentos teóricos del radar de ondas milimétricas. 

Posteriormente, el Capítulo 3 se centra en la definición del problema y en el análisis detallado de los requisitos funcionales y no funcionales del sistema. Sobre esta base, el Capítulo 4 expone el diseño de la arquitectura jerárquica en capas y modela la máquina de estados lógicos. Por su parte, la selección e integración de los componentes específicos de hardware y de las plataformas de software libre se justifica en el Capítulo 5.

El despliegue físico de los nodos y el plan de implantación se describen en el Capítulo 6, dando paso a la validación experimental y al análisis de rendimiento local en el Capítulo 7. Los aspectos económicos se detallan en el Capítulo 8 mediante la consolidación del presupuesto de ejecución, mientras que el Capítulo 9 reúne los esquemas electrónicos y planos de planta del entorno. Por último, el Capítulo 10 se sintetiza las conclusiones principales derivadas de la investigación y esboza las líneas de trabajo futuras. La memoria concluye con un conjunto de anexos que contienen los manuales técnicos y de usuario correspondientes.