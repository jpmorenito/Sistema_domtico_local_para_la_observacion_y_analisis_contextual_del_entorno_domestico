...
if os.path.exists(uploaded_img):
    with Image.open(uploaded_img) as img:
        ...
        # 2. Radar LD2410 at the bottom-left
        font_sensors = get_font(24, bold=True)
        draw.text((width // 4, height - 30), "Radar LD2410", fill="black", font=font_sensors, anchor="mm")
        
        # 3. Diodo Láser further to the right
        # We will use anchor="rm" (right-middle) and put it near the right edge
        draw.text((width - 5, height - 60), "Diodo Láser", fill="black", font=font_sensors, anchor="rm")
