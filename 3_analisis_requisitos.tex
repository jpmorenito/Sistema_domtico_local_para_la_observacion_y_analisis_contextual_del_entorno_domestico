...
\begin{itemize}
    \item \textbf{Restricciones de conectividad y latencia:} El sistema opera sobre una red local aislada para garantizar la resiliencia frente a caídas del servicio de Internet (arquitectura \textit{offline-first}). Esto exige que el orquestador y los sensores compartan el mismo dominio de difusión (dominio de \textit{broadcast}) y estén sometidos a una estricta política de red.
...
\end{figure} 
\begin{itemize}
    \item \textbf{Restricciones de hardware y procesamiento:} La orquestación central se ejecuta sobre un microordenador Raspberry Pi 4 de recursos finitos. Esto impone una limitación directa sobre la complejidad computacional algorítmica y exige un control estricto sobre el uso de la memoria RAM y el desgaste de la unidad de almacenamiento.
    \item \textbf{Restricciones de infraestructura de red:} El sistema exige cobertura ininterrumpida de red inalámbrica IEEE 802.11 (Wi-Fi) en la banda de 2.4 GHz para la transmisión de telemetría MQTT desde los nodos distribuidos, requiriendo un nivel de señal (RSSI) estable para evitar la pérdida de eventos lógicos.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{RF-03: Inferencia algorítmica de estados.} El orquestador debe procesar las entradas sensoriales para asignar unívocamente un estado lógico continuo a la estancia (ocio, estudio, durmiendo, ausente).
...
\begin{itemize}
    ...
    \item \textbf{RNF-03: Resiliencia ante caídas de red.} El servidor central debe identificar la desconexión abrupta de cualquier nodo sensor y aplicar un protocolo de contingencia mediante mecanismos de última voluntad (LWT) del protocolo MQTT.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        \node[box, right=of sensor] (broker) {Orquestador Local\\(Raspberry Pi)};
        \node[box, below=of sensor] (actuator) {Actuadores\\(Relés)};
        
        % Internet y Cloud
        \node[firewall, right=1.2cm of broker] (fw) {};
        \node[cloud, right=1.2cm of fw] (internet) {Internet /\\Cloud Externo};

        % Etiquetas
        \node[anchor=south, font=\bfseries\small, color=red] at ([yshift=0.3cm]fw.north) {Aislamiento (WPA3)};

        % Conexiones
        \draw[<->, >=stealth, thick, color=black!70] (sensor) -- node[above, font=\footnotesize] {MQTT} (broker);
        \draw[<->, >=stealth, thick, color=black!70] (broker) |- node[right, font=\footnotesize] {MQTT} (actuator);
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|c|c|}
        \hline
        \rowcolor[gray]{0.9} 
        \parbox[c]{5cm}{\vspace{0.3cm}\centering \textbf{Objetivo Específico (OE)}\vspace{0.3cm}} & \parbox[c]{4cm}{\centering \textbf{Requisitos Func. (RF)}} & \parbox[c]{4cm}{\centering \textbf{Req. No Func. (RNF)}} \\ \hline
        \parbox[c]{5cm}{\vspace{0.3cm}\centering OE-1: Diseño de hardware distribuido\vspace{0.3cm}} & RF-01, RF-02 & RNF-01 \\ \hline
        \parbox[c]{5cm}{\vspace{0.3cm}\centering OE-2: Telemetría avanzada\vspace{0.3cm}} & RF-01, RF-02 & RNF-02 \\ \hline
        \parbox[c]{5cm}{\vspace{0.3cm}\centering OE-3: Motor de contexto\vspace{0.3cm}} & RF-03 & RNF-01, RNF-04 \\ \hline
        \parbox[c]{5cm}{\vspace{0.3cm}\centering OE-4: Automatización y seguridad\vspace{0.3cm}} & RF-04, RF-05 & RNF-02, RNF-03 \\ \hline
        \parbox[c]{5cm}{\vspace{0.3cm}\centering OE-5: Validación e integración\vspace{0.3cm}} & RF-03 & RNF-03, RNF-04 \\ \hline
    \end{tabular}
\end{table}