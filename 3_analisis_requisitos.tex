...
\begin{itemize}
    ...
    \item \textbf{Restricciones de hardware y procesamiento:} la gestión centralizada central se ejecuta sobre un computador de placa única (SBC) local de recursos finitos. Esto impone una limitación directa sobre la complejidad computacional algorítmica y exige un control estricto sobre el uso de la memoria RAM y el desgaste de la unidad de almacenamiento.
...
\begin{tikzpicture}[
        ...
        \node[usecase] (cu2) at (0, 0) {Generación de\\telemetría};
...
\begin{itemize}
    ...
    \item \textbf{RNF-03: Resiliencia ante caídas de red.} El servidor central debe identificar la desconexión abrupta de cualquier nodo sensor y aplicar un protocolo de contingencia mediante el mecanismo de supervisión activa (\textit{keepalive}) de la API nativa de comunicaciones del firmware de los nodos.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
            ...
            % Nodos locales
            \node[box] (sensor) {Nodos sensores\\(microcontrolador + radar)};
            \node[box, right=of sensor] (broker) {Servidor central local\\(SBC)};
